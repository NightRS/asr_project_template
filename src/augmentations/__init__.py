...
def from_configs(configs: ConfigParser):
    wave_augs: list[Callable[[Tensor], Tensor]] = []
    ...
    spec_augs: list[Callable[[Tensor], Tensor]] = []
    if (
        "augmentations" in configs.config
        and "spectrogram" in configs.config["augmentations"]
    ):
...
def _to_function(
    augs_list: list[Callable[[Tensor], Tensor]]
